 ...
 * 广告拦截 DNS 脚本
 *
 * 基于 AWAvenue Ads Rule (v1.6.9, 2026-06-15) 的广告域名列表。
 * 当存在 direct 出站时，将匹配的广告域名解析到 0.0.0.0（IPv4）或 ::（IPv6）。
 *
...
(function () {
  ...
  // ---- 规则数据（从 AWAvenue Ads Rule 解析） ----

  // DOMAIN: 精确域名匹配
  var exactDomains = new Set([
  ...
  function matchDomain(domain) {
    ...
    // 2. 后缀匹配 (DOMAIN-SUFFIX)
    for (var i = 0; i < suffixDomains.length; i++) {
      var rule = suffixDomains[i];
      if (domain === rule || domainEndsWith(domain, '.' + rule)) {
        return true;
      }
    }

    // 3. 关键词匹配 (DOMAIN-KEYWORD)
    for (var j = 0; j < domainKeywords.length; j++) {
      if (domain.indexOf(domainKeywords[j]) !== -1) {
        return true;
      }
    }
  ...
  function domainEndsWith(domain, suffix) {
    ...
    return domain.substring(domain.length - suffix.length) === suffix;
  }
  ...
  mundo.registerHook('onProxiesLoaded', function (context) {
    ...
    if (!initialized) {
      initialized = true;
      mundo.log('ads-demo', 'AWAvenue Ads Rule loaded: ' + exactDomains.size + ' exact + ' +
        suffixDomains.length + ' suffix + ' + domainKeywords.length + ' keyword rules');
      mundo.log('ads-demo', 'direct outbound: ' + (hasDirect ? 'available' : 'not available'));
    }
  });
